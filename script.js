...
function containsBlockedLanguage(text){
  ...
  return blockedTerms.some(x=>n.includes(x.toLowerCase()));
}
...
function isAdmin(){
  return !!currentUser&&currentUser.app_metadata?.role==="admin";
}
...
async function ensureAnonymousUser(){
  ...
  currentUser=anonData?.user||null;
  return currentUser;
}
...
async function saveRemoteSettings(content,weekly){
  ...
  if(!ok){
    throw new Error("已写入但无法重新读取网站文字，请检查 site_content 的公开读取权限");
  }

  if(remoteSiteContent?.heroTitle?.zh!==content?.heroTitle?.zh){
...
async function loadRemoteArtworks(){
  ...
  remoteArtworks=(data||[]).map(x=>({
    ...
    date:(x.created_at||"").slice(0,10)||new Date().toISOString().slice(0,10),
    title:x.title||"",
    note:x.note||"",
    image:x.image_url||"",
    name:x.name||"",
...
async function loadRemoteInteractions(){
  ...
  const [g,gr,p,b,talk,a]=await Promise.all([
    supabaseClient
      ...
      .select("id,name,message,owner_id,created_at")
      .order("created_at",{ascending:false}),

    supabaseClient
      .from("guestbook_replies")
      .select("id,message_id,name,message,owner_id,created_at")
    ...
    supabaseClient
      .from("uu_talks")
      .select("id,title,text,date,created_at")
      .order("created_at",{ascending:false}),

    supabaseClient
      .from("weekly_answers")
      .select("id,name,answer,created_at")
      .order("created_at",{ascending:false})
  ]);
  ...
  if(!g.error){
    ...
    remoteGuestbook=(g.data||[]).map(x=>({
      ...x,
      replies:replies.filter(r=>r.message_id===x.id)
    }));
  }
...
function setupRealtime(){
  ...
  realtimeChannel=supabaseClient
    ...
    .on(
      "postgres_changes",
      {event:"*",schema:"public",table:"guestbook_replies"},
      loadRemoteInteractions
    )
    ...
    .on(
      "postgres_changes",
      {event:"*",schema:"public",table:"uu_talks"},
      loadRemoteInteractions
    )

    .on(
      "postgres_changes",
      {event:"*",schema:"public",table:"weekly_answers"},
      loadRemoteInteractions
    )
...
const defaultContent={
  heroTitle:{zh:"UU的小馆",en:"UU's Little Gallery"},
  heroIntro:{zh:"一页一页，收藏我画过的世界。<br>慢慢画，也慢慢长大。",en:"A little place for all the worlds I draw.<br>Drawing slowly, growing slowly."},
  coloredTitle:{zh:"有色画",en:"Color Works"},
  coloredDesc:{zh:"彩色、完整的作品，以及那些被我认真留下来的画。",en:"Finished pieces, colorful worlds, and drawings I wanted to keep."},
  sketchTitle:{zh:"随笔",en:"Sketches"},
  sketchDesc:{zh:"一些随手画下来的东西。小鸟、人物、风景、一个突然出现的念头。",en:"Little things I draw along the way — birds, people, landscapes, passing thoughts."},
  togetherTitle:{zh:"一起玩",en:"Let's Make Something Together"},
  togetherDesc:{zh:"这里不只是我的画廊。也想留一点位置，给你的想法和你的世界。",en:"This isn't only my gallery. There is a little room here for your thoughts and your world too."},
  guestbookTitle:{zh:"你在想什么呢～写下来吧",en:"What are you thinking? Write it down~"},
  privateNoteTitle:{zh:"想和UU说一点悄悄话",en:"A little private note for UU"},
  privateNoteDesc:{zh:"这里的内容不会显示在大家的小世界里，只会送到UU的管理员区。",en:"This will not appear in Everyone's Little Worlds. It goes only to UU's admin area."},
  worldsTitle:{zh:"大家的小世界",en:"Everyone's Little Worlds"},
  worldsDesc:{zh:"投稿通过审核后，会直接出现在这里。",en:"Approved submissions appear here directly."},
  uuTalkTitle:{zh:"UU·和大家说的话",en:"UU · Notes to Everyone"},
  uuTalkDesc:{zh:"这里是UU想留给大家的一些话。每一篇都可以慢慢写。",en:"A little page for notes UU wants to leave for everyone."}
};

function getContent(){
  return {
    ...defaultContent,
    ...(remoteSiteContent||getStored("uu_site_content",{}))
  };
}

function getWeekly(){
  return remoteWeeklyQuestion||
    getStored("uu_weekly_question",weeklyQuestionDefault);
}

function getMyArt(){
  return getStored("uu_my_art",[]);
}
...
function allArtworks(){
  return [
    ...baseArtworks,
    ...getMyArt(),
    ...remoteArtworks.filter(x=>x.category!=="community")
  ];
}
...
function applyContentToPage(){
  ...
  document.querySelectorAll("[data-i18n]").forEach(el=>{
    ...
    if(c[key]&&c[key][language]!==undefined){
      el.innerHTML=c[key][language];
    }
  });

  const heroTitle=
    c.heroTitle?.[language]||
    (language==="zh"?"UU的小馆":"UU's Little Gallery");

  document.querySelectorAll("[data-site-title]")
    .forEach(el=>el.textContent=heroTitle);
  ...
  if($("question-answer")){
    $("question-answer").placeholder=
      language==="zh"?q.placeholderZh:q.placeholderEn;
  }
}
...
function createSingle(x){
  return `<article class="art-card">
    <img class="art-image" src="${x.image}" alt="${escapeHtml(x.title)}" loading="lazy">
    <div class="art-info">
      <h3 class="art-title">${escapeHtml(x.title)}</h3>
      <p class="art-date">${formatDate(x.date)}</p>
      ${x.note?`<p class="art-note">${escapeHtml(x.note)}</p>`:""}
    </div>
  </article>`;
}
...
function createSeries(x){
  return `<article class="series-card">
    <div class="series-head">
      <div>
        <h3 class="series-title">${escapeHtml(x.title)}</h3>
        <p class="series-meta">${formatDate(x.date)} · ${x.images.length}${t("seriesWorks")}</p>
        ${x.note?`<p class="series-note">${escapeHtml(x.note)}</p>`:""}
      </div>
    </div>
    ...
    <div class="series-grid">
      ${x.images.map(i=>`
        <figure>
          <img class="art-image" src="${i.image}" alt="${escapeHtml(i.title||x.title)}" loading="lazy">
          <figcaption>${escapeHtml(i.title||"")}</figcaption>
        </figure>
      `).join("")}
    </div>
  </article>`;
    ...
    Object.entries(years)
      .map(([y,months])=>
        Object.entries(months)
          .map(([m,list])=>{
            const cards=list
              .map(x=>x.type==="series"?createSeries(x):createSingle(x))
              .join("");
...
function renderAll(){
  renderGallery(
    "colored-gallery",
    allArtworks().filter(x=>x.category==="colored")
  );

  renderGallery(
    "sketch-gallery",
    allArtworks().filter(x=>x.category==="sketch")
  );
...
async function initEvents(){
  ...
  $("share-preview-button")?.addEventListener(
    "click",
    ()=>$("share-preview").classList.toggle("hidden")
  );
  ...
  $("share-submit-button")?.addEventListener(
    "click",
    async()=>{
      const files=imageFiles(selectedSubmissionImage);
      const title=$("share-title").value.trim();
      const note=$("share-note").value.trim();
      const name=$("share-name").value.trim()||"匿名投稿";

      if(!files.length){
        $("share-message").textContent=t("adminNeedImage");
        return;
      }

      if(!title){
        $("share-message").textContent=t("adminNeedTitle");
        return;
      }

      if(containsBlockedLanguage(title+note)){
        $("share-message").textContent=t("unsafe");
        return;
      }
  ...
  $("guestbook-form")?.addEventListener(
    "submit",
    async e=>{
      e.preventDefault();

      const name=
        $("guest-name").value.trim()||"匿名";

      const message=
        $("guest-message").value.trim();

      if(!message){
        $("guestbook-message").textContent=t("empty");
        return;
      }

      if(containsBlockedLanguage(message)){
        $("guestbook-message").textContent=t("unsafe");
        return;
      }
  ...
  $("guestbook-list")?.addEventListener(
    "click",
    async e=>{
      const replyButton=e.target.closest("[data-reply]");
      ...
      if(del){
        ...
        try{
          const {error}=await supabaseClient
            .from("guestbook_messages")
            .delete()
            .eq("id",del.dataset.guestbookDelete)
          ...
          alert(
            language==="zh"
              ?"删除失败，这条留言可能不是你的。"
              :"Delete failed. This note may not belong to you."
          );
      ...
      const communityDel=e.target.closest(
      ...
      if(communityDel){
        ...
        if(!confirm(
          language==="zh"
            ?"确定要删除自己的投稿吗？"
            :"Delete your artwork submission?"
        ))return;

        communityDel.disabled=true;

        try{
          const id=communityDel.dataset.communityDelete;
          ...
          alert(
            language==="zh"
              ?"删除失败，这幅作品可能不是你的。"
              :"Delete failed. This artwork may not belong to you."
          );

          communityDel.disabled=false;
        }
      }
    }
  );
  ...
  $("guestbook-list")?.addEventListener(
    "submit",
    async e=>{
      if(!e.target.matches("[data-reply-form]"))return;

      e.preventDefault();

      const id=e.target.dataset.replyForm;
      const ins=e.target.querySelectorAll(
        "input,textarea"
      );
  ...
  $("question-form")?.addEventListener(
    "submit",
    async e=>{
      e.preventDefault();
      ...
      if(!answer){
        $("question-message").textContent=t("empty");
        return;
      }

      if(containsBlockedLanguage(answer)){
        $("question-message").textContent=t("unsafe");
        return;
      }
  ...
  $("private-message-form")?.addEventListener(
    "submit",
    async e=>{
      e.preventDefault();

      const name=
        $("private-name").value.trim()||"匿名";

      const message=
        $("private-message").value.trim();

      if(!message){
        $("private-message-status").textContent=t("empty");
        return;
      }

      if(containsBlockedLanguage(message)){
        $("private-message-status").textContent=t("unsafe");
        return;
      }

      if(remoteBlockedUsers.includes(name)){
        $("private-message-status").textContent=
          language==="zh"
            ?"暂时不能发送悄悄话。"
  ...
  $("admin-unlock")?.addEventListener(
    "click",
    signInAdmin
  );

  $("admin-password")?.addEventListener(
    "keydown",
    e=>{
    ...
    .forEach(tab=>{
      tab.addEventListener(
        ...
        ()=>{
          ...
          ["editor","myart","submissions","messages","talk"]
            .forEach(x=>
              $(`admin-${x}-panel`)
                ?.classList.toggle(
                  "hidden",
...
function renderGuestbook(){
  ...
  box.innerHTML=remoteGuestbook.map(x=>{
    const canDelete=
      !!currentUser&&
      !!x.owner_id&&
    ...
    return `<div class="guest-note">
      <strong>${escapeHtml(x.name||"匿名")}：</strong>
      ...
      ${
        adminCanDelete
          ?`<button
              type="button"
              class="delete-my-button"
              data-admin-guestbook-delete="${x.id}">
              ${t("adminDelete")}
            </button>`
          :""
      }
      ...
      <div class="reply-list">
        ${(x.replies||[]).map(r=>
          `<div class="reply-note">
            <strong>${escapeHtml(r.name||"匿名")}：</strong>
      ...
      <form
        ...
        <textarea
          maxlength="180"
          placeholder="${t("replyPlaceholder")}"></textarea>
  ...
  list.innerHTML=items.map(x=>
    `<article class="uu-talk-card">
      <div class="uu-talk-date">
        ${escapeHtml(formatDate(x.date||x.created_at.slice(0,10)))}
      </div>
...
async function renderAdminEditor(){
  ...
  p.innerHTML=`<div class="admin-editor-grid">
    <div>
      <h3>${t("adminText")}</h3>
      ...
      <input
        data-edit="heroTitle"
        data-lang="zh"
        value="${escapeHtml(c.heroTitle?.zh||"")}">

      <input
        data-edit="heroTitle"
        data-lang="en"
        value="${escapeHtml(c.heroTitle?.en||"")}">

      <label>${t("editText")}</label>

      <textarea data-edit="heroIntro" data-lang="zh">${escapeHtml(c.heroIntro?.zh||"")}</textarea>

      <textarea data-edit="heroIntro" data-lang="en">${escapeHtml(c.heroIntro?.en||"")}</textarea>

      <label>${t("editText")}</label>

      <textarea data-edit="coloredDesc" data-lang="zh">${escapeHtml(c.coloredDesc?.zh||"")}</textarea>

      <textarea data-edit="coloredDesc" data-lang="en">${escapeHtml(c.coloredDesc?.en||"")}</textarea>

      <textarea data-edit="sketchDesc" data-lang="zh">${escapeHtml(c.sketchDesc?.zh||"")}</textarea>

      <textarea data-edit="sketchDesc" data-lang="en">${escapeHtml(c.sketchDesc?.en||"")}</textarea>

      <textarea data-edit="togetherDesc" data-lang="zh">${escapeHtml(c.togetherDesc?.zh||"")}</textarea>

      <textarea data-edit="togetherDesc" data-lang="en">${escapeHtml(c.togetherDesc?.en||"")}</textarea>

      <textarea data-edit="guestbookTitle" data-lang="zh">${escapeHtml(c.guestbookTitle?.zh||"")}</textarea>

      <textarea data-edit="guestbookTitle" data-lang="en">${escapeHtml(c.guestbookTitle?.en||"")}</textarea>

      <textarea data-edit="privateNoteTitle" data-lang="zh">${escapeHtml(c.privateNoteTitle?.zh||"")}</textarea>

      <textarea data-edit="privateNoteTitle" data-lang="en">${escapeHtml(c.privateNoteTitle?.en||"")}</textarea>

      <textarea data-edit="privateNoteDesc" data-lang="zh">${escapeHtml(c.privateNoteDesc?.zh||"")}</textarea>

      <textarea data-edit="privateNoteDesc" data-lang="en">${escapeHtml(c.privateNoteDesc?.en||"")}</textarea>

      <textarea data-edit="worldsTitle" data-lang="zh">${escapeHtml(c.worldsTitle?.zh||"")}</textarea>

      <textarea data-edit="worldsTitle" data-lang="en">${escapeHtml(c.worldsTitle?.en||"")}</textarea>

      <textarea data-edit="worldsDesc" data-lang="zh">${escapeHtml(c.worldsDesc?.zh||"")}</textarea>

      <textarea data-edit="worldsDesc" data-lang="en">${escapeHtml(c.worldsDesc?.en||"")}</textarea>

      <textarea data-edit="uuTalkTitle" data-lang="zh">${escapeHtml(c.uuTalkTitle?.zh||"")}</textarea>

      <textarea data-edit="uuTalkTitle" data-lang="en">${escapeHtml(c.uuTalkTitle?.en||"")}</textarea>

      <textarea data-edit="uuTalkDesc" data-lang="zh">${escapeHtml(c.uuTalkDesc?.zh||"")}</textarea>

      <textarea data-edit="uuTalkDesc" data-lang="en">${escapeHtml(c.uuTalkDesc?.en||"")}</textarea>
    </div>

    <div>
      <h3>${t("editWeekly")}</h3>

      <label>${t("editQuestion")}</label>

      <input id="weekly-zh" value="${escapeHtml(getWeekly().zh||"")}">

      <input id="weekly-en" value="${escapeHtml(getWeekly().en||"")}">

      <label>${t("editPlaceholder")}</label>

      <input id="weekly-ph-zh" value="${escapeHtml(getWeekly().placeholderZh||"")}">

      <input id="weekly-ph-en" value="${escapeHtml(getWeekly().placeholderEn||"")}">

      <button id="admin-save-content" class="admin-save-main">
        ${t("adminSave")}
      </button>

      <p id="admin-content-message" class="form-message"></p>
    </div>
  </div>`;

  $("admin-save-content").addEventListener(
    "click",
    async()=>{
      const next=getContent();

      p.querySelectorAll("[data-edit]")
        .forEach(el=>{
      ...
      try{
        ...
        $("admin-content-message").textContent=
          t("adminSaved");
...
async function boot(){
  try{
      ...
      async(_event,session)=>{
        ...
        if(!currentUser){
          try{
            ...
            console.warn(
              "Anonymous session could not be restored:",
              err
            );
          }
        }
    ...
    console.error("UU boot error",err);
