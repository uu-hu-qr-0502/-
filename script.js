...
async function loadRemoteArtworks(){
  ...
  remoteArtworks=(data||[]).map(x=>({id:x.id,type:x.type||"single",category:x.category||"colored",date:(x.created_at||"").slice(0,10)||new Date().toISOString().slice(0,10),title:x.title||"",note:x.note||"",image:x.image_url||"",name:x.name||"",user_id:x.user_id||null}));
  renderAll();
}
...
async function loadRemoteInteractions(){
  ...
  const [g,gr,p,b,talk,a]=await Promise.all([
    supabaseClient.from("guestbook_messages").select("id,name,message,owner_id,created_at").order("created_at",{ascending:false}),
    supabaseClient.from("guestbook_replies").select("id,message_id,name,message,owner_id,created_at").order("created_at",{ascending:true}),
  ...
  if(!a.error)remoteWeeklyAnswers=a.data||[];
  renderGuestbook();renderUuTalks();renderWeeklyAnswerHistory();if(adminUnlocked)renderAdminMessages();
}
...
async function saveRemoteSubmission(file,title,note,name){
  ...
  const {error}=await supabaseClient.from("artwork").insert({title,image_url,note,name:name||"匿名投稿",category:"community",is_approved:false,is_owner:false,user_id:currentUser?.id||null,type:"single"});
  if(error)throw error;
}
...
async function refreshAuth(){
  ...
  if(!currentUser){
    ...
    if(anonError){console.warn("匿名登录失败",anonError);}
    else currentUser=anonData?.user||anonData?.session?.user||null;
  }
  adminUnlocked=!!currentUser&&isAdmin();updateAdminVisibility();
}
...
function renderCommunityGallery(){const box=$("community-gallery");if(!box)return;const local=getStored("uu_submissions",[]).filter(x=>x.status==="approved");const remote=remoteArtworks.filter(x=>x.category==="community");const list=[...remote,...local];box.innerHTML=list.map(x=>{const mine=!!currentUser&&x.user_id===currentUser.id;const canDelete=mine||isAdmin();return `<article class="community-card"><img src="${x.image}" alt="${escapeHtml(x.title)}" loading="lazy"><div class="community-info"><h4>${escapeHtml(x.title)}</h4><p class="community-by">${escapeHtml(x.name||"匿名投稿")}</p>${x.note?`<p>${escapeHtml(x.note)}</p>`:""}${canDelete&&x.id?`<button type="button" class="delete-my-button" data-community-delete="${x.id}">${mine?t("deleteMyArt"):t("adminDelete")}</button>`:""}</div></article>`;}).join("");$("community-empty")?.classList.toggle("hidden",list.length>0);}
...
function ensureDeleteButtonStyles(){
  if(document.getElementById("uu-delete-button-styles"))return;
  const style=document.createElement("style");
  style.id="uu-delete-button-styles";
  style.textContent=`
    .delete-my-button{display:inline-flex;align-items:center;justify-content:center;margin-left:8px;margin-top:7px;padding:4px 10px;border:1px solid rgba(201,130,148,.38);border-radius:999px;background:rgba(243,201,211,.16);color:var(--pink-deep,#b86b7d);font:inherit;font-size:.72rem;line-height:1.4;cursor:pointer;transition:background .2s ease,border-color .2s ease,transform .2s ease,opacity .2s ease;}
    .delete-my-button:hover{background:rgba(243,201,211,.38);border-color:rgba(201,130,148,.65);transform:translateY(-1px);}
    .delete-my-button:disabled{opacity:.55;cursor:wait;transform:none;}
    .reply-note .delete-my-button{margin-top:0;}
    @media(max-width:700px){.delete-my-button{margin-left:0;margin-right:6px;}}
  `;
...
async function initEvents(){
  ...
  $("guestbook-form")?.addEventListener("submit",async e=>{e.preventDefault();const name=$("guest-name").value.trim()||"匿名",message=$("guest-message").value.trim();if(!message){$("guestbook-message").textContent=t("empty");return}if(containsBlockedLanguage(message)){$("guestbook-message").textContent=t("unsafe");return}try{if(!currentUser)await refreshAuth();const {error}=await supabaseClient.from("guestbook_messages").insert({name,message,owner_id:currentUser?.id||null});if(error)throw error;await loadRemoteInteractions();$("guestbook-form").reset();$("guestbook-message").textContent=t("guestSaved");}catch(err){console.error(err);$("guestbook-message").textContent=language==="zh"?"留言没有成功，请稍后再试 ♡":"The note could not be posted. Please try again ♡";}});
  $("guestbook-list")?.addEventListener("click",async e=>{const b=e.target.closest("[data-reply]");if(b){document.querySelector(`[data-reply-form="${b.dataset.reply}"]`)?.classList.toggle("hidden");return;}const del=e.target.closest("[data-message-delete]");if(del){if(!confirm(t("deleteConfirm")))return;del.disabled=true;const {error}=await supabaseClient.from("guestbook_messages").delete().eq("id",del.dataset.messageDelete);if(error){console.error(error);del.disabled=false;return;}await loadRemoteInteractions();return;}const delReply=e.target.closest("[data-reply-delete]");if(delReply){if(!confirm(t("deleteConfirm")))return;delReply.disabled=true;const {error}=await supabaseClient.from("guestbook_replies").delete().eq("id",delReply.dataset.replyDelete);if(error){console.error(error);delReply.disabled=false;return;}await loadRemoteInteractions();}});
  $("guestbook-list")?.addEventListener("submit",async e=>{if(!e.target.matches("[data-reply-form]"))return;e.preventDefault();const id=e.target.dataset.replyForm,ins=e.target.querySelectorAll("input,textarea"),name=ins[0].value.trim()||"匿名",message=ins[1].value.trim();if(!message)return;if(containsBlockedLanguage(message)){alert(t("unsafe"));return}try{if(!currentUser)await refreshAuth();const {error}=await supabaseClient.from("guestbook_replies").insert({message_id:id,name,message,owner_id:currentUser?.id||null});if(error)throw error;await loadRemoteInteractions();}catch(err){console.error(err);alert(language==="zh"?"回复没有成功，请稍后再试。":"Reply failed. Please try again.");}});
  $("community-gallery")?.addEventListener("click",async e=>{const b=e.target.closest("[data-community-delete]");if(!b)return;if(!confirm(t("deleteConfirm")))return;b.disabled=true;const {error}=await supabaseClient.from("artwork").delete().eq("id",b.dataset.communityDelete);if(error){console.error(error);b.disabled=false;return;}await loadRemoteArtworks();});
...
function renderGuestbook(){const box=$("guestbook-list");if(!box)return;box.innerHTML=remoteGuestbook.map(x=>{const mine=!!currentUser&&x.owner_id===currentUser.id;const canDelete=mine||isAdmin();return `<div class="guest-note"><strong>${escapeHtml(x.name||"匿名")}：</strong>${escapeHtml(x.message)}<button class="reply-button" data-reply="${x.id}">${t("reply")}</button>${canDelete?`<button type="button" class="delete-my-button" data-message-delete="${x.id}">${mine?t("deleteMine"):t("adminDelete")}</button>`:""}<div class="reply-list">${(x.replies||[]).map(r=>{const replyMine=!!currentUser&&r.owner_id===currentUser.id;const replyCanDelete=replyMine||isAdmin();return `<div class="reply-note"><strong>${escapeHtml(r.name||"匿名")}：</strong>${escapeHtml(r.message)}${replyCanDelete?` <button type="button" class="delete-my-button" data-reply-delete="${r.id}">${replyMine?t("deleteMyReply"):t("adminDelete")}</button>`:""}</div>`;}).join("")}</div><form class="reply-form hidden" data-reply-form="${x.id}"><input maxlength="40" placeholder="${t("anonymous")}"><textarea maxlength="180" placeholder="${t("replyPlaceholder")}"></textarea><button type="submit">${t("sendReply")}</button></form></div>`;}).join("");}
...
(async function boot(){
  try{
    await initEvents();bindAdminDelegates();ensureDeleteButtonStyles();
    if(!supabaseClient){setAdminMessage("管理员系统没有连接到 Supabase，请检查 supabase-config.js。","error");applyLanguage();return;}
    await refreshAuth();await loadRemoteSettings();applyLanguage();await loadRemoteArtworks();await loadRemoteInteractions();setupRealtime();
