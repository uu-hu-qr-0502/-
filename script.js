/* UU的小馆 · 修正版 3.4.2
   修复：英文多行字符串导致整个 JavaScript 无法解析；同时整理管理员登录与页面初始化。
*/
...
const translations = {
  zh: {
    navSubmit:"投稿",navColored:"有色画",navSketches:"随笔",navTogether:"一起玩",navUuTalk:"UU·和大家说的话",navAbout:"关于",navAdmin:"管理员",navDirectory:"目录",
    directoryTitle:"目录",directoryDesc:"想去哪里，就点哪里吧～",directorySubmit:"把你的画放进大家的小世界",directoryColored:"看看UU认真留下来的彩色作品",directorySketches:"一些随手画下来的小念头",directoryTogether:"问题、留言、大家的小世界",directoryUuTalk:"看看UU想留给大家的话",directoryAdmin:"管理员专属的小房间",adminTalk:"UU的话",
...
const supabaseClient=(window.supabase&&window.UU_SUPABASE_URL&&window.UU_SUPABASE_KEY)
  ...
  ? window.supabase.createClient(window.UU_SUPABASE_URL,window.UU_SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true}})
  : null;
...
function getGuestToken(){
  ...
  if(!token){
    token=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():"uu-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2);
...
async function loadGuestOwnership(){
  ...
  const [g,a,w,wr]=await Promise.all([
    supabaseClient.rpc("get_my_guestbook_ids",{p_token:token}),
    supabaseClient.rpc("get_my_artwork_ids",{p_token:token}),
    supabaseClient.rpc("get_my_weekly_answer_ids",{p_token:token}),
    supabaseClient.rpc("get_my_weekly_reply_ids",{p_token:token})
  ]);
  if(!g.error)myGuestGuestbookIds=new Set((g.data||[]).map(x=>String(x.id)));
  if(!a.error)myGuestArtworkIds=new Set((a.data||[]).map(x=>String(x.id)));
  if(!w.error)myWeeklyAnswerIds=new Set((w.data||[]).map(x=>String(x.id)));
  if(!wr.error)myWeeklyReplyIds=new Set((wr.data||[]).map(x=>String(x.id)));
}
async function deleteMyGuestbookMessage(id){
  const {data,error}=await supabaseClient.rpc("delete_my_guestbook_message",{p_id:String(id),p_token:getGuestToken()});
  if(error)throw error;
  if(!data)throw new Error("这条留言不是当前浏览器发布的");
}
async function deleteMyArtwork(id){
  const {data,error}=await supabaseClient.rpc("delete_my_artwork",{p_id:String(id),p_token:getGuestToken()});
  if(error)throw error;
  if(!data)throw new Error("这幅画不是当前浏览器投稿的");
}

async function deleteMyWeeklyAnswer(id){
  const {data,error}=await supabaseClient.rpc("delete_my_weekly_answer",{p_id:String(id),p_token:getGuestToken()});
  if(error)throw error;
  if(!data)throw new Error("这条答案不是当前浏览器发布的");
}

async function deleteMyWeeklyReply(id){
  const {data,error}=await supabaseClient.rpc("delete_my_weekly_reply",{p_id:String(id),p_token:getGuestToken()});
  if(error)throw error;
  if(!data)throw new Error("这条回复不是当前浏览器发布的");
...
function imageFiles(files){return Array.from(files||[]).filter(f=>f&&f.type.startsWith("image/"));}
...
async function uploadGalleryImage(file,folder){
  ...
  if(error)throw error;
  return supabaseClient.storage.from("gallery-images").getPublicUrl(path).data.publicUrl;
}

async function loadRemoteSettings(){
  if(!supabaseClient)return false;
  const {data,error}=await supabaseClient.from("site_content").select("content,weekly,updated_at").eq("id",1).maybeSingle();
  if(error){console.warn("Supabase site content read failed",error);return false;}
  if(data){remoteSiteContent=data.content||{};remoteWeeklyQuestion=data.weekly||{};window.UU_REMOTE_CONTENT_UPDATED_AT=data.updated_at||null;return true;}
  return false;
}

async function saveRemoteSettings(content,weekly){
  if(!supabaseClient||!isAdmin())throw new Error("管理员未登录");
  const {error}=await supabaseClient.from("site_content").upsert({id:1,content,weekly,updated_at:new Date().toISOString()},{onConflict:"id"});
  if(error)throw error;
  const ok=await loadRemoteSettings();
  if(!ok)throw new Error("已写入但无法重新读取网站文字，请检查 site_content 的公开读取权限");
  if(remoteSiteContent?.heroTitle?.zh!==content?.heroTitle?.zh)throw new Error("数据库里的首页标题没有更新成功，请检查 site_content 表");
}
...
async function loadRemoteInteractions(){
  if(!supabaseClient)return;
  const [g,gr,p,b,talk,a,waReplies]=await Promise.all([
    ...
    supabaseClient.from("weekly_answer_replies").select("id,answer_id,name,message,created_at").order("created_at",{ascending:true})
  ]);
  if(!g.error){const replies=gr.error?[]:(gr.data||[]);remoteGuestbook=(g.data||[]).map(x=>({...x,replies:replies.filter(r=>r.message_id===x.id)}));}
  if(!p.error)remotePrivateMessages=p.data||[];
  if(!b.error)remoteBlockedUsers=(b.data||[]).map(x=>x.name);
  if(!talk.error)remoteUuTalks=talk.data||[];
  if(!a.error)remoteWeeklyAnswers=(a.data||[]).map(x=>({...x,replies:waReplies.error?[]:(waReplies.data||[]).filter(r=>r.answer_id===x.id)}));
  await loadGuestOwnership();renderGuestbook();renderUuTalks();renderWeeklyAnswerHistory();if(adminUnlocked)renderAdminMessages();
}
...
function setupRealtime(){
  ...
  realtimeChannel=supabaseClient.channel("uu-global-sync")
    .on("postgres_changes",{event:"*",schema:"public",table:"site_content"},async()=>{await loadRemoteSettings();applyContentToPage();})
    ...
    .on("postgres_changes",{event:"*",schema:"public",table:"guestbook_messages"},loadRemoteInteractions)
    .on("postgres_changes",{event:"*",schema:"public",table:"guestbook_replies"},loadRemoteInteractions)
    .on("postgres_changes",{event:"*",schema:"public",table:"private_messages"},loadRemoteInteractions)
    .on("postgres_changes",{event:"*",schema:"public",table:"blocked_users"},loadRemoteInteractions)
    .on("postgres_changes",{event:"*",schema:"public",table:"uu_talks"},loadRemoteInteractions)
    .on("postgres_changes",{event:"*",schema:"public",table:"weekly_answers"},loadRemoteInteractions)
    .on("postgres_changes",{event:"*",schema:"public",table:"weekly_answer_replies"},loadRemoteInteractions)
    .subscribe();
}

async function saveRemoteSubmission(file,title,note,name){
  const image_url=await uploadGalleryImage(file,"submissions");
  const {error}=await supabaseClient.from("artwork").insert({title,image_url,note,name:name||"匿名投稿",category:"community",is_approved:false,is_owner:false,type:"single",owner_token:getGuestToken()});
  if(error)throw error;
}

async function refreshAuth(){
  if(!supabaseClient){setAdminMessage("管理员系统连接失败：Supabase 配置没有加载。","error");return;}
  const {data}=await supabaseClient.auth.getSession();
  currentUser=data?.session?.user||null;adminUnlocked=!!currentUser&&isAdmin();updateAdminVisibility();
}

function setAdminMessage(msg,type=""){
  const el=$("admin-message");if(!el)return;el.textContent=msg;el.dataset.state=type;
}
...
function updateAdminVisibility(){
  ...
  section.classList.toggle("admin-private-hidden",!open);section.classList.toggle("admin-visible",open);
...
async function signInAdmin(){
  ...
  if(!email||!password){setAdminMessage("请输入管理员邮箱和密码 ♡");return;}
  if(!supabaseClient){setAdminMessage("管理员系统没有连接到 Supabase，请检查配置文件。","error");return;}
  const button=$("admin-unlock");if(button)button.disabled=true;setAdminMessage("正在登录……");
  try{
    const {data,error}=await supabaseClient.auth.signInWithPassword({email,password});
    if(error)throw error;
    currentUser=data.user;
    if(!isAdmin()){
      await supabaseClient.auth.signOut();currentUser=null;adminUnlocked=false;
      setAdminMessage("这个账号还没有管理员权限。请在 Supabase 的 App Metadata 里确认 role=admin。","error");
      return;
    }
    adminUnlocked=true;setAdminMessage("登录成功 ♡");
    $("admin-locked")?.classList.add("hidden");$("admin-panel")?.classList.remove("hidden");
    if($("admin-password"))$("admin-password").value="";
    await loadRemoteSettings();await loadRemoteArtworks();await loadRemoteInteractions();renderAdminAll();
  }catch(err){
    console.error(err);setAdminMessage(`登录没有成功：${err?.message||"请检查邮箱和密码"}`,"error");
...
const defaultContent={
  ...
  worldsTitle:{zh:"大家的小世界",en:"Everyone's Little Worlds"},worldsDesc:{zh:"投稿通过审核后，会直接出现在这里。",en:"Approved submissions appear here directly."},
  uuTalkTitle:{zh:"UU·和大家说的话",en:"UU · Notes to Everyone"},uuTalkDesc:{zh:"这里是UU想留给大家的一些话。每一篇都可以慢慢写。",en:"A little page for notes UU wants to leave for everyone."}
};
...
function applyContentToPage(){
  const c=getContent();
  document.querySelectorAll("[data-i18n]").forEach(el=>{const key=el.dataset.i18n;if(c[key]&&c[key][language]!==undefined)el.innerHTML=c[key][language];});
  ...
  const q=getWeekly();if($("question-title"))$("question-title").innerHTML=language==="zh"?q.zh:q.en;if($("question-answer"))$("question-answer").placeholder=language==="zh"?q.placeholderZh:q.placeholderEn;
}
function applyLanguage(){
  ...
  if($("language-toggle"))$("language-toggle").textContent=language==="zh"?"EN":"中";
...
function renderGallery(id,items){const box=$(id);if(!box)return;const years=groupByYear(items);const html=Object.entries(years).map(([y,months])=>Object.entries(months).map(([m,list])=>{const cards=list.map(x=>x.type==="series"?createSeries(x):createSingle(x)).join("");const monthName=new Date(m+"-01T12:00:00").toLocaleDateString(language==="zh"?"zh-CN":"en-US",{month:"long"});return `<div class="month-block"><h4>${monthName}</h4><div class="art-grid">${cards}</div></div>`;}).join("")).join("");box.innerHTML=html||`<p class="admin-empty">${t("adminEmpty")}</p>`;}
function renderCommunityGallery(){const box=$("community-gallery");if(!box)return;const local=getStored("uu_submissions",[]).filter(x=>x.status==="approved");const remote=remoteArtworks.filter(x=>x.category==="community");const list=[...remote,...local];box.innerHTML=list.map(x=>`<article class="community-card"><img src="${x.image}" alt="${escapeHtml(x.title)}" loading="lazy"><div class="community-info"><h4>${escapeHtml(x.title)}</h4><p class="community-by">${escapeHtml(x.name||"匿名投稿")}</p>${x.note?`<p>${escapeHtml(x.note)}</p>`:""}${myGuestArtworkIds.has(String(x.id))?`<button class="danger-button" data-guest-art-delete="${x.id}">${t("deleteMy")}</button>`:""}</div></article>`).join("");$("community-empty")?.classList.toggle("hidden",list.length>0);}
...
async function initEvents(){
  ...
  $("share-image")?.addEventListener("change",e=>{const files=imageFiles(e.target.files);if(!files.length)return;selectedSubmissionImage=files;$("share-image-preview").innerHTML=files.map((f,i)=>`<div class="upload-thumb"><img src="${URL.createObjectURL(f)}" alt="preview ${i+1}"></div>`).join("");$("share-image-preview").classList.remove("hidden");});
  $("share-submit-button")?.addEventListener("click",async()=>{const files=imageFiles(selectedSubmissionImage),title=$("share-title").value.trim(),note=$("share-note").value.trim(),name=$("share-name").value.trim()||"匿名投稿";if(!files.length){$("share-message").textContent=t("adminNeedImage");return}if(!title){$("share-message").textContent=t("adminNeedTitle");return}if(containsBlockedLanguage(title+note)){$("share-message").textContent=t("unsafe");return}const b=$("share-submit-button");b.disabled=true;try{if(!supabaseClient)throw new Error("Supabase 未连接");for(const file of files)await saveRemoteSubmission(file,title,note,name);$("share-message").textContent=language==="zh"?`收到啦 ♡ ${files.length} 幅已经送进UU的审核小房间。`:`Got it ♡ ${files.length} artwork(s) are waiting for UU's review.`;$("share-name").value=$("share-title").value=$("share-note").value="";$("share-image").value="";selectedSubmissionImage=null;$("share-image-preview").classList.add("hidden");}catch(err){console.error(err);$("share-message").textContent=language==="zh"?"投稿没有成功，请稍后再试 ♡":"The submission could not be uploaded. Please try again ♡";}finally{b.disabled=false;if(adminUnlocked)renderAdminAll();}});

  $("guestbook-form")?.addEventListener("submit",async e=>{e.preventDefault();const name=$("guest-name").value.trim()||"匿名",message=$("guest-message").value.trim();if(!message){$("guestbook-message").textContent=t("empty");return}if(containsBlockedLanguage(message)){$("guestbook-message").textContent=t("unsafe");return}try{const {error}=await supabaseClient.from("guestbook_messages").insert({name,message,owner_token:getGuestToken()});if(error)throw error;await loadRemoteInteractions();$("guestbook-form").reset();$("guestbook-message").textContent=t("guestSaved");}catch(err){console.error(err);$("guestbook-message").textContent=language==="zh"?"留言没有成功，请稍后再试 ♡":"The note could not be posted. Please try again ♡";}});
  $("guestbook-list")?.addEventListener("click",async e=>{const del=e.target.closest("[data-guest-delete]");if(del){if(!confirm(language==="zh"?"确定要删除这条留言吗？":"Delete this message?"))return;try{del.disabled=true;await deleteMyGuestbookMessage(del.dataset.guestDelete);await loadRemoteInteractions();}catch(err){console.error(err);alert(language==="zh"?"这条留言不能由当前浏览器删除。":"This message cannot be deleted from this browser.");}finally{del.disabled=false;}return;}const b=e.target.closest("[data-reply]");if(b)document.querySelector(`[data-reply-form="${b.dataset.reply}"]`)?.classList.toggle("hidden");});
  $("community-gallery")?.addEventListener("click",async e=>{const b=e.target.closest("[data-guest-art-delete]");if(!b)return;if(!confirm(language==="zh"?"确定要删除这幅投稿吗？":"Delete this artwork?"))return;try{b.disabled=true;await deleteMyArtwork(b.dataset.guestArtDelete);await loadRemoteArtworks();}catch(err){console.error(err);alert(language==="zh"?"这幅画不能由当前浏览器删除。":"This artwork cannot be deleted from this browser.");}finally{b.disabled=false;}});
  ...
  $("question-form")?.addEventListener("submit",async e=>{e.preventDefault();const answer=$("question-answer").value.trim();if(!answer){$("question-message").textContent=t("empty");return}if(containsBlockedLanguage(answer)){$("question-message").textContent=t("unsafe");return}try{const {error}=await supabaseClient.from("weekly_answers").insert({name:"匿名",answer,owner_token:getGuestToken()});if(error)throw error;$("question-answer").value="";await loadRemoteInteractions();$("question-message").textContent=t("guestSaved");}catch(err){console.error(err);$("question-message").textContent=language==="zh"?"答案没有送出去，请稍后再试。":"The answer could not be sent. Please try again.";}});

  $("weekly-answer-history")?.addEventListener("click",async e=>{
    const replyButton=e.target.closest("[data-weekly-reply]");
    if(replyButton){document.querySelector(`[data-weekly-reply-form="${replyButton.dataset.weeklyReply}"]`)?.classList.toggle("hidden");return;}
    const del=e.target.closest("[data-weekly-delete]");
    if(del){if(!confirm(language==="zh"?"确定要删除这条答案吗？":"Delete this answer?"))return;try{del.disabled=true;if(isAdmin()){const {data,error}=await supabaseClient.rpc("admin_delete_weekly_answer",{p_id:String(del.dataset.weeklyDelete)});if(error)throw error;if(!data)throw new Error("管理员删除失败");}else{await deleteMyWeeklyAnswer(del.dataset.weeklyDelete);}await loadRemoteInteractions();}catch(err){console.error(err);alert(language==="zh"?"这条答案不能删除。":"This answer cannot be deleted.");}finally{del.disabled=false;}return;}
    const delReply=e.target.closest("[data-weekly-reply-delete]");
    if(delReply){if(!confirm(language==="zh"?"确定要删除这条回复吗？":"Delete this reply?"))return;try{delReply.disabled=true;if(isAdmin()){const {data,error}=await supabaseClient.rpc("admin_delete_weekly_reply",{p_id:String(delReply.dataset.weeklyReplyDelete)});if(error)throw error;if(!data)throw new Error("管理员删除失败");}else{await deleteMyWeeklyReply(delReply.dataset.weeklyReplyDelete);}await loadRemoteInteractions();}catch(err){console.error(err);alert(language==="zh"?"这条回复不能删除。":"This reply cannot be deleted.");}finally{delReply.disabled=false;}return;}
  });

  $("weekly-answer-history")?.addEventListener("submit",async e=>{
    if(!e.target.matches("[data-weekly-reply-form]"))return;
    e.preventDefault();
    const id=e.target.dataset.weeklyReplyForm;
    const ins=e.target.querySelectorAll("input,textarea");
    const name=ins[0].value.trim()||"匿名",message=ins[1].value.trim();
    if(!message)return;
    if(containsBlockedLanguage(message)){alert(t("unsafe"));return;}
    try{const {error}=await supabaseClient.from("weekly_answer_replies").insert({answer_id:id,name,message,owner_token:getGuestToken()});if(error)throw error;await loadRemoteInteractions();}catch(err){console.error(err);alert(language==="zh"?"回复没有成功，请稍后再试。":"Reply failed. Please try again.");}
  });
  ...
  $("admin-unlock")?.addEventListener("click",signInAdmin);
  $("admin-password")?.addEventListener("keydown",e=>{if(e.key==="Enter")signInAdmin();});
...
function renderGuestbook(){const box=$("guestbook-list");if(!box)return;box.innerHTML=remoteGuestbook.map(x=>`<div class="guest-note"><strong>${escapeHtml(x.name||"匿名")}：</strong>${escapeHtml(x.message)}<button class="reply-button" data-reply="${x.id}">${t("reply")}</button>${myGuestGuestbookIds.has(String(x.id))?`<button class="reply-button danger-button" data-guest-delete="${x.id}">${t("deleteMy")}</button>`:""}<div class="reply-list">${(x.replies||[]).map(r=>`<div class="reply-note"><strong>${escapeHtml(r.name||"匿名")}：</strong>${escapeHtml(r.message)}</div>`).join("")}</div><form class="reply-form hidden" data-reply-form="${x.id}"><input maxlength="40" placeholder="${t("anonymous")}"><textarea maxlength="180" placeholder="${t("replyPlaceholder")}"></textarea><button type="submit">${t("sendReply")}</button></form></div>`).join("");}
function renderUuTalks(){const list=$("uu-talk-list"),empty=$("uu-talk-empty");if(!list)return;const items=remoteUuTalks;list.innerHTML=items.map(x=>`<article class="uu-talk-card"><div class="uu-talk-date">${escapeHtml(formatDate(x.date||x.created_at.slice(0,10)))}</div><h3>${escapeHtml(x.title)}</h3><p>${escapeHtml(x.text).replace(/\n/g,"<br>")}</p></article>`).join("");empty?.classList.toggle("hidden",items.length>0);}
function renderWeeklyAnswerHistory(){const box=$("weekly-answer-history");if(!box)return;const cutoff=Date.now()-14*24*60*60*1000;const items=remoteWeeklyAnswers.filter(x=>new Date(x.created_at).getTime()>=cutoff);box.innerHTML=items.map(x=>{const mine=myWeeklyAnswerIds.has(String(x.id));const canDelete=mine||isAdmin();return `<div class="weekly-answer-item"><span>${escapeHtml(x.name||"匿名")}</span><p>${escapeHtml(x.answer)}</p><div class="weekly-answer-actions"><button type="button" class="reply-button" data-weekly-reply="${x.id}">${t("reply")}</button>${canDelete?`<button type="button" class="danger-button" data-weekly-delete="${x.id}">${t("deleteMy")}</button>`:""}</div><div class="weekly-answer-replies">${(x.replies||[]).map(r=>{const replyMine=myWeeklyReplyIds.has(String(r.id));const replyCanDelete=replyMine||isAdmin();return `<div class="weekly-answer-reply"><strong>${escapeHtml(r.name||"匿名")}：</strong>${escapeHtml(r.message)}${replyCanDelete?`<button type="button" class="danger-button" data-weekly-reply-delete="${r.id}">${t("deleteMy")}</button>`:""}</div>`}).join("")}</div><form class="reply-form hidden" data-weekly-reply-form="${x.id}"><input maxlength="40" placeholder="${t("anonymous")}"><textarea maxlength="180" placeholder="${t("replyPlaceholder")}"></textarea><button type="submit">${t("sendReply")}</button></form></div>`}).join("");}
...
function renderAdminEditor(){const q=getWeekly();const p=$("admin-editor-panel");if(!p)return;p.innerHTML=`<div class="admin-editor-grid"><div class="editor-section"><h3>${t("editHero")}</h3>${field("heroTitle",t("editTitle"))}${field("heroIntro",t("editText"),true)}</div><div class="editor-section"><h3>${t("editGallery")}</h3>${field("coloredTitle",t("colored")+" · "+t("editTitle"))}${field("coloredDesc",t("editText"),true)}${field("sketchTitle",t("sketch")+" · "+t("editTitle"))}${field("sketchDesc",t("editText"),true)}</div><div class="editor-section"><h3>${t("editTogether")}</h3>${field("togetherTitle",t("editTitle"))}${field("togetherDesc",t("editText"),true)}${field("guestbookTitle",t("editTitle"))}${field("privateNoteTitle",t("editTitle"))}${field("privateNoteDesc",t("editText"),true)}${field("worldsTitle",t("editTitle"))}${field("worldsDesc",t("editText"),true)}${field("uuTalkTitle",t("editUuTalk"))}${field("uuTalkDesc",t("editText"),true)}</div><div class="editor-section weekly-editor"><h3>${t("editWeekly")}</h3><div class="editor-lang"><div><label>中文</label><textarea id="weekly-zh" class="large">${escapeHtml(q.zh)}</textarea><textarea id="weekly-ph-zh">${escapeHtml(q.placeholderZh)}</textarea></div><div><label>English</label><textarea id="weekly-en" class="large">${escapeHtml(q.en)}</textarea><textarea id="weekly-ph-en">${escapeHtml(q.placeholderEn)}</textarea></div></div></div></div><button class="admin-save-main" id="save-site-content">${t("adminSave")}</button><p id="admin-content-message" class="form-message"></p>`;$("save-site-content").addEventListener("click",async()=>{const c=getContent();document.querySelectorAll("[data-content-edit]").forEach(el=>{c[el.dataset.contentEdit]??={zh:"",en:""};c[el.dataset.contentEdit][el.dataset.lang]=el.value;});const weekly={zh:$("weekly-zh").value,en:$("weekly-en").value,placeholderZh:$("weekly-ph-zh").value,placeholderEn:$("weekly-ph-en").value};saveStored("uu_site_content",c);saveStored("uu_weekly_question",weekly);try{await saveRemoteSettings(c,weekly);applyContentToPage();$("admin-content-message").textContent=t("adminSaved");}catch(err){console.error(err);$("admin-content-message").textContent=(language==="zh"?"保存失败：":"Save failed: ")+(err?.message||"未知错误");}});}

async function renderAdminMyArt(){const p=$("admin-myart-panel");if(!p)return;let items=[];if(supabaseClient&&isAdmin()){const {data,error}=await supabaseClient.from("artwork").select("id,created_at,title,image_url,category,note,type,user_id").eq("is_owner",true).eq("user_id",currentUser.id).order("created_at",{ascending:false});if(!error)items=(data||[]).map(x=>({id:x.id,image:x.image_url,title:x.title||"",category:x.category||"colored",date:(x.created_at||"").slice(0,10),note:x.note||""}));}p.innerHTML=`<div class="my-art-form"><h3>${t("adminMyArt")}</h3><label>${t("editImage")}</label><label class="upload-box" for="my-art-image"><span class="upload-icon">＋</span><span>${t("uploadPhoto")}</span></label><input id="my-art-image" class="visually-hidden" type="file" accept="image/*"><div id="my-art-preview" class="upload-preview hidden"></div><label>${t("editName")}</label><input id="my-art-title"><label>${t("editCategory")}</label><select id="my-art-category"><option value="colored">${t("colored")}</option><option value="sketch">${t("sketch")}</option></select><label>${t("editDate")}</label><input id="my-art-date" type="date" value="${new Date().toISOString().slice(0,10)}"><label>${t("editNote")}</label><textarea id="my-art-note"></textarea><button id="add-my-art" class="admin-save-main">${t("adminAddArt")}</button><p id="my-art-message" class="form-message"></p></div><div class="admin-own-list">${items.map(x=>`<article class="admin-own-item"><img src="${x.image}" alt="${escapeHtml(x.title)}"><div><h4>${escapeHtml(x.title)}</h4><p>${x.category==="colored"?t("colored"):t("sketch")} · ${escapeHtml(x.date)}</p>${x.note?`<p>${escapeHtml(x.note)}</p>`:""}<button class="danger-button" data-own-delete="${x.id}">${t("adminDelete")}</button></div></article>`).join("")||`<p class="admin-empty">${t("adminEmpty")}</p>`}</div>`;$("my-art-image").addEventListener("change",e=>{selectedMyArtImage=e.target.files?.[0]||null;if(selectedMyArtImage){$("my-art-preview").innerHTML=`<img src="${URL.createObjectURL(selectedMyArtImage)}" alt="preview">`;$("my-art-preview").classList.remove("hidden");}});$("add-my-art").addEventListener("click",async()=>{const title=$("my-art-title").value.trim(),f=selectedMyArtImage;if(!f){$("my-art-message").textContent=t("adminNeedImage");return}if(!title){$("my-art-message").textContent=t("adminNeedTitle");return}try{const image=await uploadGalleryImage(f,"owner");const {error}=await supabaseClient.from("artwork").insert({title,image_url:image,category:$("my-art-category").value,note:$("my-art-note").value.trim(),is_approved:true,is_owner:true,user_id:currentUser.id,type:"single"});if(error)throw error;selectedMyArtImage=null;await loadRemoteArtworks();await renderAdminMyArt();$("my-art-message").textContent=t("adminArtSaved");}catch(err){console.error(err);$("my-art-message").textContent=language==="zh"?"上传没有成功，请检查 Supabase 权限。":"Upload failed. Please check Supabase permissions.";}});p.onclick=async e=>{const b=e.target.closest("[data-own-delete]");if(!b)return;const {error}=await supabaseClient.from("artwork").delete().eq("id",b.dataset.ownDelete);if(error){console.error(error);return}await loadRemoteArtworks();renderAdminMyArt();};}

async function renderAdminSubmissions(){const p=$("admin-submissions-panel");if(!p)return;if(!isAdmin()){p.innerHTML="";return}const {data,error}=await supabaseClient.from("artwork").select("id,created_at,title,image_url,category,is_approved,note,name").eq("is_owner",false).order("created_at",{ascending:false});if(error){p.innerHTML=`<p class="admin-empty">${language==="zh"?"暂时读不到投稿，请检查 RLS。":"Could not load submissions. Check RLS."}</p>`;return}p.innerHTML=(data||[]).map(x=>`<article class="admin-item"><img src="${x.image_url}" alt="${escapeHtml(x.title||"")}"><div><div class="admin-item-top"><span class="status-pill ${x.is_approved?"approved":"pending"}">${x.is_approved?t("adminApproved"):t("adminPending")}</span><small>${new Date(x.created_at).toLocaleString()}</small></div><h4>${escapeHtml(x.title||"")}</h4><p class="admin-author">${escapeHtml(x.name||"匿名投稿")}</p>${x.note?`<p>${escapeHtml(x.note)}</p>`:""}<div class="admin-actions">${!x.is_approved?`<button data-remote-action="approve" data-id="${x.id}">${t("adminApprove")}</button>`:""}<button class="danger-button" data-remote-action="delete" data-id="${x.id}">${t("adminDelete")}</button></div></div></article>`).join("")||`<p class="admin-empty">${t("adminEmpty")}</p>`;}

async function renderAdminMessages(){const p=$("admin-messages-panel");if(!p)return;const items=remotePrivateMessages,blocked=remoteBlockedUsers;p.innerHTML=`<div class="admin-private-tools"><strong>${t("blockedUsers")}</strong><div class="blocked-list">${blocked.map(n=>`<span class="blocked-chip">${escapeHtml(n)} <button type="button" data-unblock-user="${escapeHtml(n)}">×</button></span>`).join("")||`<span class="small-note">${t("noBlockedUsers")}</span>`}</div></div>`+(items.map(x=>{const isBlocked=blocked.includes(x.name||"匿名");return `<article class="private-admin-note ${isBlocked?"is-blocked":""}"><div class="admin-item-top"><span class="status-pill private">${t("adminPrivate")}</span><small>${new Date(x.created_at).toLocaleString()}</small></div><h4>${escapeHtml(x.name||"匿名")}</h4><p>${escapeHtml(x.message)}</p>${x.reply?`<div class="admin-reply-preview"><strong>${t("yourReply")}</strong><p>${escapeHtml(x.reply)}</p></div>`:""}<div class="private-actions"><button type="button" data-message-reply="${x.id}">${t("replyToMessage")}</button><button type="button" data-block-user="${escapeHtml(x.name||"匿名")}">${isBlocked?t("unblockUser"):t("blockUser")}</button><button type="button" class="danger-button" data-message-delete="${x.id}">${t("adminDelete")}</button></div><form class="private-reply-form hidden" data-reply-message-form="${x.id}"><textarea maxlength="500" placeholder="${t("replyMessagePlaceholder")}">${escapeHtml(x.reply||"")}</textarea><button type="submit" class="admin-save-main">${t("sendAdminReply")}</button></form></article>`}).join("")||`<p class="admin-empty">${t("adminNoMessages")}</p>`);}

async function renderAdminTalks(){const p=$("admin-talk-panel");if(!p)return;const talks=remoteUuTalks;p.innerHTML=`<h3>${t("editUuTalk")}</h3><label>${t("talkTitle")}</label><input id="talk-title" maxlength="80" placeholder="${t("talkTitlePlaceholder")}"><label>${t("talkText")}</label><textarea id="talk-text" maxlength="1000" placeholder="${t("talkTextPlaceholder")}"></textarea><label>${t("talkDate")}</label><input id="talk-date" type="date" value="${new Date().toISOString().slice(0,10)}"><button id="add-uu-talk" class="admin-save-main">${t("publishTalk")}</button><div class="admin-talk-list">${talks.map(x=>`<article><div><strong>${escapeHtml(x.title)}</strong><small>${escapeHtml(x.date||"")}</small><p>${escapeHtml(x.text)}</p></div><button class="danger-button" data-talk-delete="${x.id}">${t("adminDelete")}</button></article>`).join("")}</div>`;$("add-uu-talk").onclick=async()=>{const title=$("talk-title").value.trim(),text=$("talk-text").value.trim();if(!title||!text)return;const {error}=await supabaseClient.from("uu_talks").insert({title,text,date:$("talk-date").value});if(error){console.error(error);return}await loadRemoteInteractions();renderAdminTalks();};p.onclick=async e=>{const b=e.target.closest("[data-talk-delete]");if(!b)return;const {error}=await supabaseClient.from("uu_talks").delete().eq("id",b.dataset.talkDelete);if(error)console.error(error);await loadRemoteInteractions();renderAdminTalks();};}

function renderAdminAll(){renderAdminEditor();renderAdminMyArt();renderAdminSubmissions();renderAdminMessages();renderAdminTalks();}

function bindAdminDelegates(){
  $("admin-submissions-panel")?.addEventListener("click",async e=>{const b=e.target.closest("[data-remote-action]");if(!b||!isAdmin())return;const id=b.dataset.id;if(b.dataset.remoteAction==="approve"){await supabaseClient.from("artwork").update({is_approved:true}).eq("id",id);}else if(b.dataset.remoteAction==="delete"){await supabaseClient.from("artwork").delete().eq("id",id);}await loadRemoteArtworks();renderAdminSubmissions();});
  $("admin-messages-panel")?.addEventListener("click",async e=>{const del=e.target.closest("[data-message-delete]");if(del){await supabaseClient.from("private_messages").delete().eq("id",del.dataset.messageDelete);await loadRemoteInteractions();return}const rep=e.target.closest("[data-message-reply]");if(rep){document.querySelector(`[data-reply-message-form="${rep.dataset.messageReply}"]`)?.classList.toggle("hidden");return}const block=e.target.closest("[data-block-user]");if(block){const name=block.dataset.blockUser;if(remoteBlockedUsers.includes(name))await supabaseClient.from("blocked_users").delete().eq("name",name);else await supabaseClient.from("blocked_users").insert({name});await loadRemoteInteractions();return}const unblock=e.target.closest("[data-unblock-user]");if(unblock){await supabaseClient.from("blocked_users").delete().eq("name",unblock.dataset.unblockUser);await loadRemoteInteractions();}});
  $("admin-messages-panel")?.addEventListener("submit",async e=>{if(!e.target.matches("[data-reply-message-form]"))return;e.preventDefault();const id=e.target.dataset.replyMessageForm,text=e.target.querySelector("textarea").value.trim();if(!text)return;await supabaseClient.from("private_messages").update({reply:text,replied_at:new Date().toISOString()}).eq("id",id);await loadRemoteInteractions();});
}

(async function boot(){
  try{
    await initEvents();bindAdminDelegates();
    if(!supabaseClient){setAdminMessage("管理员系统没有连接到 Supabase，请检查 supabase-config.js。","error");applyLanguage();return;}
    await loadRemoteSettings();applyLanguage();await loadRemoteArtworks();await loadRemoteInteractions();setupRealtime();await refreshAuth();
    supabaseClient.auth.onAuthStateChange(async(_event,session)=>{currentUser=session?.user||null;adminUnlocked=!!currentUser&&isAdmin();updateAdminVisibility();if(adminUnlocked){await loadRemoteSettings();applyLanguage();renderAdminAll();}});
  }catch(err){console.error("UU boot error",err);setAdminMessage("页面初始化失败，请刷新一次。","error");}
})();
