...
function groupByYear(items){
  const years={};
  [...items].sort((a,b)=>b.date.localeCompare(a.date)).forEach(x=>{
    const y=x.date.slice(0,4),m=x.date.slice(0,7);
    years[y]??={};
    years[y][m]??=[];
    years[y][m].push(x);
  });
...
function renderGallery(id,items){
  ...
  const html=Object.entries(years).map(([y,months])=>Object.entries(months).map(([m,list])=>{
...
function renderCommunityGallery(){
  ...
  if(!box)return;
  const local=getStored("uu_submissions",[]).filter(x=>x.status==="approved");
  const remote=remoteArtworks.filter(x=>x.category==="community");
  const list=[...remote,...local];
  box.innerHTML=list.map(x=>`<article class="community-card"><img src="${x.image}" alt="${escapeHtml(x.title)}" loading="lazy"><div class="community-info"><h4>${escapeHtml(x.title)}</h4><p class="community-by">${escapeHtml(x.name||"匿名投稿")}</p>${x.note?`<p>${escapeHtml(x.note)}</p>`:""}</div></article>`).join("");
  $("community-empty")?.classList.toggle("hidden",list.length>0);
}

function renderAll(){
  renderGallery("colored-gallery",allArtworks().filter(x=>x.category==="colored"));
  renderGallery("sketch-gallery",allArtworks().filter(x=>x.category==="sketch"));
...
function renderUuTalks(){const list=$("uu-talk-list"),empty=$("uu-talk-empty");if(!list)return;const items=remoteUuTalks;list.innerHTML=items.map(x=>`<article class="uu-talk-card"><div class="uu-talk-date">${escapeHtml(formatDate(x.date||x.created_at.slice(0,10)))}</div><h3>${escapeHtml(x.title)}</h3><p>${escapeHtml(x.text)}</p></article>`).join("");empty?.classList.toggle("hidden",items.length>0);}

function renderWeeklyAnswerHistory(){
  const box=$("weekly-answer-history");
  if(!box)return;
  const items=remoteWeeklyAnswers.slice(0,14);
  box.innerHTML=items.map(x=>`<div class="answer-note"><strong>${escapeHtml(x.name||"匿名")}：</strong>${escapeHtml(x.answer)}</div>`).join("");
}

async function renderAdminEditor(){
  const p=$("admin-editor-panel");
  if(!p||!isAdmin())return;
  const c=getContent();
  p.innerHTML=`<div class="admin-editor-grid"><div><h3>${t("adminText")}</h3><label>${t("editTitle")}</label><input data-edit="heroTitle" data-lang="zh" value="${escapeHtml(c.heroTitle?.zh||"")}"><input data-edit="heroTitle" data-lang="en" value="${escapeHtml(c.heroTitle?.en||"")}"><label>${t("editText")}</label><textarea data-edit="heroIntro" data-lang="zh">${escapeHtml(c.heroIntro?.zh||"")}</textarea><textarea data-edit="heroIntro" data-lang="en">${escapeHtml(c.heroIntro?.en||"")}</textarea><label>${t("editText")}</label><textarea data-edit="coloredDesc" data-lang="zh">${escapeHtml(c.coloredDesc?.zh||"")}</textarea><textarea data-edit="coloredDesc" data-lang="en">${escapeHtml(c.coloredDesc?.en||"")}</textarea><textarea data-edit="sketchDesc" data-lang="zh">${escapeHtml(c.sketchDesc?.zh||"")}</textarea><textarea data-edit="sketchDesc" data-lang="en">${escapeHtml(c.sketchDesc?.en||"")}</textarea><textarea data-edit="togetherDesc" data-lang="zh">${escapeHtml(c.togetherDesc?.zh||"")}</textarea><textarea data-edit="togetherDesc" data-lang="en">${escapeHtml(c.togetherDesc?.en||"")}</textarea><textarea data-edit="guestbookTitle" data-lang="zh">${escapeHtml(c.guestbookTitle?.zh||"")}</textarea><textarea data-edit="guestbookTitle" data-lang="en">${escapeHtml(c.guestbookTitle?.en||"")}</textarea><textarea data-edit="privateNoteTitle" data-lang="zh">${escapeHtml(c.privateNoteTitle?.zh||"")}</textarea><textarea data-edit="privateNoteTitle" data-lang="en">${escapeHtml(c.privateNoteTitle?.en||"")}</textarea><textarea data-edit="privateNoteDesc" data-lang="zh">${escapeHtml(c.privateNoteDesc?.zh||"")}</textarea><textarea data-edit="privateNoteDesc" data-lang="en">${escapeHtml(c.privateNoteDesc?.en||"")}</textarea><textarea data-edit="worldsTitle" data-lang="zh">${escapeHtml(c.worldsTitle?.zh||"")}</textarea><textarea data-edit="worldsTitle" data-lang="en">${escapeHtml(c.worldsTitle?.en||"")}</textarea><textarea data-edit="worldsDesc" data-lang="zh">${escapeHtml(c.worldsDesc?.zh||"")}</textarea><textarea data-edit="worldsDesc" data-lang="en">${escapeHtml(c.worldsDesc?.en||"")}</textarea><textarea data-edit="uuTalkTitle" data-lang="zh">${escapeHtml(c.uuTalkTitle?.zh||"")}</textarea><textarea data-edit="uuTalkTitle" data-lang="en">${escapeHtml(c.uuTalkTitle?.en||"")}</textarea><textarea data-edit="uuTalkDesc" data-lang="zh">${escapeHtml(c.uuTalkDesc?.zh||"")}</textarea><textarea data-edit="uuTalkDesc" data-lang="en">${escapeHtml(c.uuTalkDesc?.en||"")}</textarea></div><div><h3>${t("editWeekly")}</h3><label>${t("editQuestion")}</label><input id="weekly-zh" value="${escapeHtml(getWeekly().zh||"")}"><input id="weekly-en" value="${escapeHtml(getWeekly().en||"")}"><label>${t("editPlaceholder")}</label><input id="weekly-ph-zh" value="${escapeHtml(getWeekly().placeholderZh||"")}"><input id="weekly-ph-en" value="${escapeHtml(getWeekly().placeholderEn||"")}"><button id="admin-save-content" class="admin-save-main">${t("adminSave")}</button><p id="admin-content-message" class="form-message"></p></div></div>`;
  $("admin-save-content").addEventListener("click",async()=>{
    const next=getContent();
    p.querySelectorAll("[data-edit]").forEach(el=>{
      next[el.dataset.edit]??={};
      next[el.dataset.edit][el.dataset.lang]=el.value;
    });
    const weekly={zh:$("weekly-zh").value,en:$("weekly-en").value,placeholderZh:$("weekly-ph-zh").value,placeholderEn:$("weekly-ph-en").value};
    saveStored("uu_site_content",next);
    saveStored("uu_weekly_question",weekly);
    try{
      await saveRemoteSettings(next,weekly);
      applyContentToPage();
      $("admin-content-message").textContent=t("adminSaved");
    }catch(err){
      console.error(err);
      $("admin-content-message").textContent=(language==="zh"?"保存失败：":"Save failed: ")+(err?.message||"未知错误");
    }
  });
}
