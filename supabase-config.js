...
function renderGuestbook(){const box=$("guestbook-list");if(!box)return;box.innerHTML=remoteGuestbook.map(x=>`<div class="guest-note"><strong>${escapeHtml(x.name||"匿名")}：</strong>${escapeHtml(x.message)}<button class="reply-button" data-reply="${x.id}">${t("reply")}</button><div class="reply-list">${(x.replies||[]).map(r=>`<div class="reply-note"><strong>${escapeHtml(r.name||"匿名")}：</strong>${escapeHtml(r.message)}</div>`).join("")}</div><form class="reply-form hidden" data-reply-form="${x.id}"><input maxlength="40" placeholder="${t("anonymous")}"><textarea maxlength="180" placeholder="${t("replyPlaceholder")}"></textarea><button type="submit">${t("sendReply")}</button></form></div>`).join("");}
